...
(() => {
  ...
  const API_BASE = ""; // same-origin; endpoint /api/status, /api/repos, /api/deploy
  ...
  function renderStatusUI(){
    ...
    const cardMap = [
      ...
      { sel: ".api-card:nth-child(2)", data: STATUS.vercel,  hint: "VERCEL_DEPLOY_HOOK" },
      { sel: ".api-card:nth-child(3)", data: STATUS.netlify, hint: "NETLIFY_BUILD_HOOK" },
    ];
  ...
  async function loadRealRepos(){
    ...
    try {
      ...
      if (data.error){
        console.warn("[wanzz-deploy] /api/repos error:", data.error, data.detail);
        return;
      }
      ...
      console.warn("[wanzz-deploy] Gagal memuat /api/repos:", err);
    }
  }
  ...
  const LOADING_STEPS = [
    ...
    { at: 70, msg: "MENGAMBIL REPOSITORY...",             log: "[api] GET /api/repos" },
  ...
  function startRenameRepo(card){
    ...
    function finishRename(){
      ...
      // repo asli di GitHub. Untuk repo hasil /api/repos, deploy tetap
  ...
  /* ---------- proses deploy ASLI ----------
     Memanggil /api/deploy di server, yang meneruskan permintaan ke
     Vercel Deploy Hook / Netlify Build Hook sungguhan. Ini benar-benar
     memicu build & deploy di akun Vercel/Netlify yang sudah kamu
     hubungkan lewat environment variable server (lihat README-BACKEND.md). */
  async function runDeploySequence(target, btn){
    ...
    if (!keyInfo || !keyInfo.configured){
      ...
      pushConsoleLine(`  Set environment variable ${targetKey === "vercel" ? "VERCEL_DEPLOY_HOOK" : "NETLIFY_BUILD_HOOK"} di dashboard Vercel project ini, lalu redeploy.`, "warn");
    ...
    const repoName = selectedRepo || `${githubUser}/(belum pilih repo)`;

    pushConsoleLine(`$ wanzz deploy --target=${targetKey}`, "");
    await delay(200);
    pushConsoleLine(`Repository terpilih: ${repoName}`, "dim");
    await delay(300);
    pushConsoleLine("Memanggil server (POST /api/deploy)...", "info");
    await delay(200);
    ...
    try {
      const res = await fetch(`${API_BASE}/api/deploy`, {
        ...
        body: JSON.stringify({ target: targetKey, repoFullName: selectedRepo }),
      });
      const data = await res.json();

      if (!res.ok || !data.ok){
        pushConsoleLine(`✗ Deploy gagal: ${data.error || res.statusText}`, "err");
        if (data.detail) pushConsoleLine("  " + JSON.stringify(data.detail).slice(0, 200), "dim");
      } else {
        pushConsoleLine(`✓ Hook diterima oleh ${target} (server merespons OK)`, "ok");
        await delay(300);
        pushConsoleLine("Build & deploy sekarang berjalan di infrastruktur " + target + ".", "info");
        await delay(250);
        pushConsoleLine(`✓ Cek progres & URL live di dashboard ${target} kamu — pantau dari sana, bukan dari sini.`, "ok");
      }
    } catch (err) {
      pushConsoleLine("✗ Gagal menghubungi /api/deploy — pastikan situs ini dideploy sebagai Vercel project (bukan dibuka sebagai file lokal).", "err");
